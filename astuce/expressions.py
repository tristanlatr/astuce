...
from typing import Any, Callable, Iterator, Optional, Sequence, List, Tuple, Type, Union
...
class Name:
    ...
    def __init__(self, source: str, full: str | Callable[[], str]) -> None:
        """Initialize the name.
        Parameters:
            source: The name as written in the source code.
        ...
        if isinstance(full, str):
            self._full: str = full
            self._resolver: Callable[[], str] = lambda: ''
        else:
            self._full = ""
            self._resolver = full
...
class Expression(list): #type: ignore
    ...
    def tuple_item(self, nth: int) -> str | Name:
        ...
        #       N , N , N
        #       0 1 2 3 4
        return self[2][2 * nth] #type:ignore [no-any-return]
